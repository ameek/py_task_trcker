...
class ReportService {
    ...
    static async getCompletionStats(userId) {
        const allTasks = await Task.find({ user_id: userId }).toArray();
    ...
    static async exportData(userId, format = 'json') {
        const tasks = await Task.find({ user_id: userId }).toArray();
        const timeSessions = await TimeSession.find({ user_id: userId }).toArray();
